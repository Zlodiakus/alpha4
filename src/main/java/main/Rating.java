...
public class Rating {
    public static String getFullRate(){
        ...
        try {
            ...
            PreparedStatement stmt=con.prepareStatement("select  p.Name ,p.Level ,p.Gold ,p.Exp ,count(c.Distance) cor_count,sum(c.Distance) sum_distance,Max(c.Distance) max_distance,round(avg(c.Distance)) avg_distance, p.Race, sum(c.profit) profit from Players p left join Caravans c on (p.GUID=c.PGUID and c.Finish is not null) WHERE p.Name !=  'Elf' group by p.Name,p.Level,p.Gold,p.Exp order by Exp desc");
            ...
            while (rs.next()){
                ...
                result=result+"<tr class="+ccolor+"><td>"+i+"</td><td><b>"+rs.getString(1)+"</b></td><td>"+rs.getInt(2)+"</td><td>"+rs.getInt(3)+"</td><td>"+rs.getInt(4)+"</td><td>"+rs.getInt(5)+"</td><td>"+rs.getInt(6)+"</td><td>"+rs.getInt(7)+"</td><td>"+rs.getInt(8)+"</td><td>"+rs.getInt(10)+"</td></tr>";
            }
            //result=result+"</table>";
            con.close();
